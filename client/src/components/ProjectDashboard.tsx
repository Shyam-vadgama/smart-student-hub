...
import { Github, ExternalLink, Edit, Trash2, CheckCircle, XCircle, Clock, Upload, Link as LinkIcon, Settings, Rocket } from "lucide-react";
...
const ProjectDashboard: React.FC = () => {
  ...
  const handleDeployToVercel = (projectId: string) => {
    deployToVercelMutation.mutate(projectId);
  };
                  ...
                  <TableBody>
                    {projects.map((project) => (
                      <TableRow key={project._id}>
                        ...
                        <TableCell>
                          <div className="flex gap-2">
                            {/* Show Deploy to Vercel button if GitHub deployed but not Vercel */}
                            ...
                            {/* Show Redeploy button if already deployed to Vercel */}
                            {project.vercelUrl && project.deploymentStatus !== 'Pending' && (
                              <Button
                                size="sm"
                                variant="outline"
                                onClick={() => handleDeployToVercel(project._id)}
          ...
          <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
            <DialogContent>
              ...
              <DialogFooter>
                ...
                <Button
                  variant="destructive"
                  onClick={() => selectedProject && deleteProjectMutation.mutate(selectedProject._id)}
